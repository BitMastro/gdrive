...
func PrintUrl(d *gdrive.Drive, fileId string, download bool) {
	info, err := d.Files.Get(fileId).Do()
	if err != nil {
		fmt.Printf("An error occurred: %v\n", err)
		return
	...
	if download {
		...
		fmt.Println(info.AlternateLink)
	}
}

// Upload file to drive
func Upload(d *gdrive.Drive, input io.ReadCloser, title string, share bool, folder string) {
	...
	if folder != "" {
		caller := d.Files.List()
		q := fmt.Sprintf("title = '%s' and mimeType = 'application/vnd.google-apps.folder'", folder)
		caller.MaxResults(int64(1))
		caller.Q(q)
		list, err := caller.Do()
		if err != nil {
			fmt.Println(err)
	...
	if folderId != "" {
		metadata.Parents = []*drive.ParentReference{&drive.ParentReference{
			Kind: "drive#fileLink",
			Id:   folderId,
		}}
	}
...
// Create a new folder
func Mkdir(d *gdrive.Drive, title string, share bool) string {
	...
	info, err := d.Files.Insert(metadata).Do()
	if err != nil {
		fmt.Printf("An error occurred making the folder: %v\n", err)
		return ""
	}
	...
	// Print information about uploaded file
	printInfo(d, info)
	fmt.Printf("Created folder '%s' at %s, total %s\n", info.Title, getRate(bytes), util.FileSizeFormat(bytes))
